...
public class car : MonoBehaviour {
	...
	public float lowestSpeedAtSteer = 20;
	...
	public float MaxBackwardSpeed=35;

	public bool braked;
	private bool backWardBrake;
	...
	void Start ()
	{
		...
		rot=steerWheel.transform.localRotation;
    ...
    {
		// Axis will return 0->1 for triggers
		return Input.GetAxis("Right Trigger") - Input.GetAxis("Left Trigger"); 
	}

	private bool GetHandBrakeButton()
    {
		return Input.GetButton("B Button");
    }

	//private float GetMoveBackwards()
	//{
	//	Debug.Log("Left Trigger " + Input.GetAxis("Left Trigger"));
	//	return Input.GetAxis("Left Trigger");
	//}

	private bool IsCameraSwitchPressed()
	{
	...
	void control (){
		...
		torque = maxTorque * movementDirection;

		if(movementDirection < 0 && speed>MaxBackwardSpeed)
			torque=0;
	...
	void HandBrake()
	{	
		if ( GetHandBrakeButton() )
        {
			braked = true;
        }
		//else if(((GetMovementDirection()==0 && (int)speed==0) || backWardBrake))
		//{
		//	braked=true;
		//}
		else
