...
public class Controller : MonoBehaviour
{
    ...
    [Header("Audio")]
    public RandomAudioClipPlayer FootstepAudioPlayer;
    ...
    {
        return m_CurrentWeapon > -1 && m_CurrentWeapon < m_Weapons.Count;
    }
    ...
    void Update()
    {
        if (CanPause && Input.GetButtonDown("Menu"))
        {
            ...
            Debug.LogWarning("No menu to display in game");
        }
        ...
        if (!m_IsPaused && !LockControl)
        {
            ...
            if (m_Grounded && Input.GetButtonDown("Jump"))
            {
                ...
                m_Grounded = false;
                loosedGrounding = true;
                FootstepAudioPlayer.PlayClip(JumpingAudioCLip, 0.8f,1.1f);
            }

            var weaponState = Weapon.WeaponState.Idle;
            if (HasWeaponEquipped())
            {
                weaponState = m_Weapons[m_CurrentWeapon].CurrentState;
            }

            bool running = weaponState == Weapon.WeaponState.Idle && Input.GetButton("Run");
        ...
        {
            FootstepAudioPlayer.PlayClip(LandingAudioClip, 0.8f,1.1f);
        }
    }
    ...
    {
        FootstepAudioPlayer.PlayRandom();
    }
}
